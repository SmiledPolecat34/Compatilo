...
export function createApp() {
  ...
  app.use(
    pinoHttp({
      logger,
      autoLogging: { ignore: (req) => req.url === '/api/health' || req.url === '/health' },
    }),
  );
  app.use(express.json({ limit: '512kb' })); // signatures PNG en data URL
  app.use(cookieParser());

  // Convention d'infra (Render, etc.) : `/health` sans préfixe, en plus de
  // l'API sous `/api`.
  app.use('/api/health', healthRouter);
  app.use('/health', healthRouter);
  app.use('/api/public', monitoringRouter);
